...
def get_explainer():
    """
    훈련 데이터를 사용해 LIME 설명 도구를 준비한다.
    직렬화하지 않아도 될만큼 충분히 빠르다.
...
def simplify_order_sign(order_sign):
    """
    사용자에게 명확한 출력을 위해 기호를 단순화 한다.
    Args:
        order_sign: 비교 연산자 입력
    Return:
        단순화된 연산자
    """
    if order_sign in ["<=", "<"]:
...
def get_recommended_modification(simple_order, impact):
    """
    연산자와 영향 타입에 따라 추천 문장을 생성한다.

    Args:
        simple_order: 단순화된 연산자
        impact: 변화가 긍정적인지 부정적인지 여부
...
def parse_explanations(exp_list):
    """
    LIME이 반환한 설명을 사용자가 읽을 수 있도록 파싱한다.

    Args:
        exp_list: LIME 설명 도구가 반환한 설명

    Return:
        사용자에게 전달한 문자열을 담은 딕셔너리 배열
    """
    parsed_exps = []
    for (
        feat_bound,
        ...
        exp_list
    ):  # feat_bound = "{feature_name} < threshold" or "threshold < {feature_name} < threshold" , impact = float
        conditions = feat_bound.split(" ")

        # 추천으로 표현하기 힘들기 때문에
        # 1 <= a < 3 와 같은 이중 경계 조건은 무시한다.
        if len(conditions) == 3:
